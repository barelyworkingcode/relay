...
func moduleRoot(t *testing.T) string {
	...
	// This file lives at <root>/presence/presence_seam_test.go.
	root := filepath.Dir(filepath.Dir(file))
	if _, err := os.Stat(filepath.Join(root, "go.mod")); err != nil {
		t.Fatalf("presence: computed module root %q has no go.mod: %v", root, err)
	}
	return root
}
...
// in the module imports relaygo/presence/presencetest. Go links only what
...
func TestPresence_SeamIsNotLinkedIntoTheBinary(t *testing.T) {
	...
	for _, path := range nonTestSourceFiles(t, root, ".go") {
		...
		for _, imp := range f.Imports {
			...
			if p == "relaygo/presence/presencetest" {
				t.Errorf("%s imports relaygo/presence/presencetest, which must never reach a shipped binary", path)
			}
		}
	}
}
...
func TestPresencetest_HasAnInitGuard(t *testing.T) {
	root := moduleRoot(t)
	path := filepath.Join(root, "presence", "presencetest", "presencetest.go")
	src := readFile(t, path)
...
func TestPresence_ProbeTakesAnExplicitPeerFD(t *testing.T) {
	...
	for _, name := range []string{"session_darwin.go", "session_other.go"} {
		path := filepath.Join(root, "presence", name)
		src := readFile(t, path)
		for _, f := range forbidden {
			if strings.Contains(src, f) {
