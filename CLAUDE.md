...
- `relay eve enrol` — opens a five-minute, single-use window for one new browser to register an *eve* passkey ([`docs/eve-passkey-enrolment.md`](docs/eve-passkey-enrolment.md)). Brokered like `relay login enrol`, over `admin_op("eve.enrolment.open")`; the tray's **Allow Eve Passkey Enrolment…** item reaches the same gated core method. Relay only holds the window (`Settings.EveEnrolment`) and answers eve's status/consume routes — the passkeys themselves live in eve's own `auth.json`, never here.
