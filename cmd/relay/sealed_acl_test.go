...
func TestPresence_GateCannotReachTheKeychain(t *testing.T) {
	presenceDir := filepath.Join(repoRoot(t), "internal", "presence")
	...
	for _, e := range entries {
		...
		for _, imp := range f.Imports {
			...
			if path == "github.com/barelyworkingcode/relay/internal/sealed" {
				t.Errorf("%s imports github.com/barelyworkingcode/relay/internal/sealed — presence.Gate.Require would have a path to the keychain", name)
			}
		}
	}
}
...
func TestSeal_NoDevModeACLRelaxation(t *testing.T) {
	...
	entries, err := os.ReadDir(relaySourceDir(t))
	if err != nil {
	...
	for _, e := range entries {
		...
		scan(filepath.Join(relaySourceDir(t), name))
	}
	for _, sub := range []string{filepath.Join("internal", "sealed")} {
