...
// HelperCDHash and HelperTeam are set by build.sh via -ldflags -X, in the
// order SP3 verified: build+sign Contents/Helpers/relay-sessions, read its
// CDHash, then build relay with that hash embedded (`R/build.sh`'s helper
// step, before the existing `go build -o relay` line). HelperTeam is the
