...
// ---------------------------------------------------------------------------
// The real, unmodified relay-sessions binary never registers its manifest.
// ---------------------------------------------------------------------------

// TestRealRelaySessionsBinary_NeverRegistersItsManifest drives the actual,
// unmodified `relay-sessions service` binary through a real service.Registry
// launch -- the identical shape BuiltinRelaySessionsService gives it in
// production (internal/service/builtin_sessions.go: "service",
// "-internal-socket", "-hook-socket", nothing else) -- and confirms it Hellos
// onto relay's real bridge socket (proving the process really started and
// really tried) but never calls RegisterManifest, so relay's
// EnhancedServiceRegistry never learns how to reach it. Every other test in
// this file uses TestHelperSessionHost instead, a test-only stand-in that
// adds the one call this binary is missing (this file's package comment) --
// this is the one test that touches the real binary's real main(), to prove
// the gap is real rather than an artifact of the stand-in.
func TestRealRelaySessionsBinary_NeverRegistersItsManifest(t *testing.T) {
	relaySessionsBin, _ := rs10Binaries(t)
	...
	sockDir := mkShortTempDir(t, "rs10-real-bin-")
	logDir := mkShortTempDir(t, "rs10-real-bin-log-")
	registry := service.NewRegistry()
	registry.Launches = launches
	registry.Enhanced = enhanced
	registry.OpenLog = func(id string) (io.WriteCloser, error) {
	...
	// shape, not a shape convenient for this test.
	cfg := &config.ServiceConfig{
		ID: config.RelaySessionsServiceID, DisplayName: "Session Host", Command: relaySessionsBin,
		Args: []string{
			"service",
			"-internal-socket", filepath.Join(sockDir, "internal.sock"),
	...
	if _, ok := launches.Bound(config.RelaySessionsServiceID); !ok {
		t.Fatal("the real relay-sessions binary never even Hello'd onto the bridge -- this test cannot distinguish 'never registers' from 'never started'")
	}

	// It Hello'd; give it every chance to also register before concluding it
	// never will.
	time.Sleep(3 * time.Second)
	if es := enhanced.Get(config.RelaySessionsServiceID); es != nil {
		t.Fatalf("the real relay-sessions binary DID register a manifest (%+v) -- TestHelperSessionHost's extra RegisterManifest call, and every test relying on it, is now a redundant no-op the production binary already does; this test (and the skip helper the other properties use) can be deleted", es)
	}
}
