...
// swallowed (a disk hiccup must never stall the PTY readLoop).
func (l *terminalLogger) Write(p []byte) {
	...
	// only the most recent tailCapBytes are kept. A truncation marker is
...
// requested terminal ID. HTTP callers map this to 404.
...
// the number of files removed; errors on individual files don't abort the
// sweep.
func SweepTerminalLogs(dir string, maxAge time.Duration, maxTotalBytes int64) (removed int, err error) {
	...
	for _, e := range entries {
		...
		// dir is unusual but the sweep must still fail safely.
		name := e.Name()
...
// traversal when serving a log over HTTP. Terminal IDs are minted as UUIDs
// by relay (C5's session_id), so the shape is fixed.
