...
// test that reaches the bridge socket path (RunMCPServer's client, or a
// bridge server) cannot touch the real ~/Library/Application Support/relay.
//
...
func TestMain(m *testing.M) {
	home, err := os.MkdirTemp("", "mcp-test-home-")
	if err != nil {
		fmt.Fprintln(os.Stderr, "mcp tests: create sandbox home:", err)
		os.Exit(1)
	}
	os.Setenv("HOME", home)
