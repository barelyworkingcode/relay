...
//
// It is a leaf package so that internal/presence (which derives a caller's
// audit session from the token) and internal/bridge (which binds a launch
// identity to it) read the token through one implementation rather than two.
package peertoken
