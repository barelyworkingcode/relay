...
- Project update approval is re-checked against the live record inside the
  ...
  fetch, so a schema lost between the prompt and the write, after the gate
  ignored a stored derived field, surfaces as an unapproved `context`
  widening and is refused rather than written, unless the approval already
  covered `context`.
