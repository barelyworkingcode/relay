...
- **HTTP (frontend socket, for eve)**
  ...
  Eve's launch identity, holding the `frontend` capability, has `read`, `configure`,
  `proxy` and `execute`, so both are reachable with no `Authorization` header. They are registered
