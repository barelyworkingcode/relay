...
3. **Skill emission via bridge call (manual)**:
   ...
   relay skills regen --project <name> --path ~/Documents/EVE/ACME/.claude/skills/relay
   ls ~/Documents/EVE/ACME/.claude/skills/relay/SKILL.md
   grep '<token-plaintext>' ~/Documents/EVE/ACME/.claude/skills/relay/SKILL.md   # must return zero hits
   ```
4. **Pi.Dev end-to-end** (the headline test):
   ...
   - Open eve in a project context (e.g. `ACME` with `Path: ~/Documents/EVE/ACME`), hit shell launcher → expect `Pi.dev` card → click.
   - Verify `~/Documents/EVE/ACME/.claude/skills/relay/SKILL.md` was just written (mtime within seconds of click).
